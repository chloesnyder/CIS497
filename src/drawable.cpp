...
Drawable::Drawable(GLWidget277* context)
    : bufIdx(), bufPos(), bufNor(), bufCol(), bufVertData(),
      idxBound(false), posBound(false), norBound(false), colBound(false), vertDataBound(false),
      context(context)
{}
...
void Drawable::destroy()
{
    ...
    context->glDeleteBuffers(1, &bufVertData);
}
...
void Drawable::generateVertData()
{
    vertDataBound = true;
    // Create a VBO on our GPU and store its handle in bufCol
    context->glGenBuffers(1, &bufVertData);
}
